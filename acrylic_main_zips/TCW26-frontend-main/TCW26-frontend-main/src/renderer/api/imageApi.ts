...
function getNetworkErrorMessage(error: any): string {
  ...
  if (error instanceof TypeError && error.message.includes('fetch')) {
    return 'Cannot connect to the image processing server. Please ensure the backend is running on http://localhost:8000'
  }
...
export async function checkBackendHealth(): Promise<{ online: boolean; message: string }> {
  console.log('[imageApi] Checking backend health at', API_BASE_URL)
  const controller = new AbortController()
  ...
  try {
    ...
    return { 
      online: false, 
      message: `Backend is not reachable at ${API_BASE_URL}` 
    }
  }
}
